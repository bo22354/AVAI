...
parser.add_argument("--data-aug-perspect", action="store_true")
...
def main(args):
    ...
    if(args.data_aug_perspect):
        transformList.append(transforms.RandomPerspective())

    
    transformList.append(transforms.ColorJitter(brightness = args.data_aug_brightness))
...
class CNN(nn.Module):
    def __init__(self, height: int, width: int, channels: int, class_count: int, dropout:float):
        ...
        self.conv1norm2 = nn.BatchNorm2d(32)
        ...
        self.conv2norm2 = nn.BatchNorm2d(64)
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        x = F.relu(self.conv1norm2(self.conv1(images)))
        x = self.pool1(x)
        ## TASK 2-2: Pass x through the second convolutional layer
        x = F.relu(self.conv2norm2(self.conv2(x)))
...
class Trainer:
    ...
    def validate(self):
        ...
        for c in range(len(cifar)):
            print(f"{cifar[c]} accuracy: {classAcc[c] * 100:2.2f}")
...
def compute_per_class_accuracy(labels: Union[torch.Tensor, np.ndarray], preds: Union[torch.Tensor, np.ndarray]) -> [float]:
    ...
    acc = np.array([0 for c in cifar], dtype=float)
